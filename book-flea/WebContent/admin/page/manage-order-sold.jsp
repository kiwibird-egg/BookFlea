...
<body>
	<main>
		...
		<div class="page-title">
			<a href="?sec=order">주문관리</a>&nbsp>&nbsp<a href="#"> 판매완료 도서</a>
		</div>
			...
			<%	
				...
				String sellerId = DAO.getSoldInfo(orderNo).getSellerId();
				String bookTitle = DAO.getSoldInfo(orderNo).getBookTitle();
				String bookCondition = DAO.getSoldInfo(orderNo).getBookCondition();
				int price = DAO.getSoldInfo(orderNo).getPrice();
				String buyerId = DAO.getSoldInfo(orderNo).getBuyerId();
				String soldDate = DAO.getSoldInfo(orderNo).getSoldDate();
			...
			%>	<form action="/BOOKFLEA/admin/action/order-allow.jsp" class="book-edit-form">			
					...
					<ul>
						...
						<li>
							<label>구매자</label>
							<input type="text" value="<%=buyerId %>" name="publisher" disabled>
						</li>
						<li>
							<label>거래완료일</label>
							<input type="text" value="<%=soldDate %>" name="publisher" disabled>
						</li>
					</ul>
					<div class="book-edit-form__btns">
						<div class="book-edit-form__btns__left">
							...
							<!-- <button type="button" class="btn-white" onclick="location.href='/BOOKFLEA/admin/action/order-denied.jsp?orderNo=<%=orderNo %>'" style="margin-left:10px;">판매취소</button>
							 -->
						</div>
						<div class="book-edit-form__btns__right">						
							<!-- 
							<button type="submit" class="btn-green">판매취소</button>
							 -->
						</div>
					</div>
				</form>
